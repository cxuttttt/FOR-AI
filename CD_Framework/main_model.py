...
# from models.backbone.spectformer import spectformer_b # 历史backbone

from models.backbone.focalnet import FocalNet # 目前在用的backbone
...
class ChangeDetection(nn.Module):
    def __init__(self, opt):
        super().__init__()
        self.inplanes = int(re.sub(r"\D", "", opt.backbone.split("_")[-1]))  # backbone的名称中必须在"_"之后加上它的通道数 # 通道数传入
        # print(self.inplanes )
        ...
        # self.check_channels = ChannelChecker(self.backbone, self.inplanes, opt.input_size) # 实例化models.block里Base.py里对齐通道的类:ChannelChecker() <--> train里backbone_后给的通道
    ...
    def forward_once(self, xa, xb):
        _, _, h_input, w_input = xa.shape # 张量赋值(b、c、h、w):batch-size,channel,高,宽
        assert xa.shape == xb.shape, "The two images are not the same size, please check it."  # xa,xb张量shape的检查
        # 如果xa与xb尺寸不同,则打印AssertionError与"The two images are not the same size, please check it.(两个尺寸不同)"

        fa1, fa2, fa3, fa4 = self.backbone(xa)  # feature_a_1: 输入图像a的最大输出特征图
        # 前时序的多尺度特征进特征编码器backbone
        ...
        # fa1, fa2, fa3, fa4 = self.check_channels(fa1, fa2, fa3, fa4) # 对齐train里backbone_后给的通道
        # print("111",fa1.shape,fa2.shape,fa3.shape,fa4.shape)
        
        fb1, fb2, fb3, fb4 = self.backbone(xb)
        # 后时序的多吃度特征进特征编码器backbone
        # print(fb1.shape,fb2.shape,fb3.shape,fb4.shape)
        # torch.Size([32, 96, 56, 56]) torch.Size([32, 192, 28, 28]) torch.Size([32, 384, 14, 14]) torch.Size([32, 768, 7, 7])

        # fb1, fb2, fb3, fb4 = self.check_channels(fb1, fb2, fb3, fb4) # 对齐train里backbone_后给的通道
        # print("222",fa1.shape,fa2.shape,fa3.shape,fa4.shape)

        ms_feats = fa1, fa2, fa3, fa4, fb1, fb2, fb3, fb4   # 以tuple类型存储多尺度特征

        change = self.neck(ms_feats) # 多尺度特征进neck进行融合
        # print("11",fa1.shape)
        # print("44",fa4.shape)
        # print("cc",change.shape)
    ...
    def _create_backbone(self, backbone): # 换backbone
        if 'efficientnet' in backbone:
            ...
            if '_t_' in backbone:
                ...
                self. backbone = FocalNet() # 调用backbone中的模型类Focalnet（）
        
        # elif 'spectformer' in backbone:
        #         self. backbone = spectformer_b()
