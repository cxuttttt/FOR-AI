...
def eval_for_metric(model, eval_loader, criterion=None, tta=False, input_size=512):   # 评估,得到指标(metric)
    ...
    p, r, f1, miou, oa = compute_p_r_f1_miou_oa(tn_fp_fn_tp) # 指标调用 , 换指标
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckp-paths", type=str,
                        default=[
                            "D:/compressed_file/CD_Framework/runs/train/2/best_ckp/focalnet_96_fpn+aspp+fuse+drop_fcn_epoch_1_0.37066.pt",
                        ])  # 测试权重

    parser.add_argument("--cuda", type=str, default="0")  # GPU编号
    parser.add_argument("--dataset-dir", type=str, default="/mnt/Disk1/BCD_Dataset/LEVIR-CD/")
